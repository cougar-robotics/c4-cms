...
function slugify (schema, options) {
  ...
  , 'method'
    ...
    schema[method]('slugify', function (str) {
      ...
      // Get a relatively unique number to avoid conflicts
      var epoch = '' + (new Date).getTime()
      var rand = Math.floor(Math.random() * 1000)
      var randstr = epoch.substr(epoch.length - 3) + rand + '-'

      // Replace all invalid characters and spaces, truncating to the max length
      return ((randstr + str)
        .replace(/\s*@\s*/, ' at ') // the \s's are to eliminate extra whitespace
        .replace(/\s*&\s*/, ' and ')
        .replace(/[^a-z0-9-_+ ]/g, invalidChar)
